...
class KMoveAcquisitionService:
    def __init__(self):
        ...
        self._calibration_duration = 2.0
        self._csv_path = None
    ...
    def start(self, duration=30.0, filename=None):
        with self._lock:
            ...
            if self._recording or self._armed or self._calibrating:
            ...
            self._calibration_started_at = now_iso()
            self._test_offsets = {
                "rotation_x_deg": 0.0,
                "rotation_y_deg": 0.0,
                "rotation_z_deg": 0.0,
        ...
        with self._lock:
            ...
            if row and status["phase"] in {"ready", "armed", "active"}:
                measurement = {
                    ...
                    "rotation_deg": self._adjusted_value(row, "rotation_x_deg"),
                    "flexion_extension_deg": self._adjusted_value(
                        row,
                        "rotation_y_deg",
                    ),
                    "inclination_deg": self._adjusted_value(
                        row,
                        "rotation_z_deg",
                    ),
    ...
    def _should_trigger(self, row):
        try:
            values = (
                abs(self._adjusted_value(row, "rotation_x_deg")),
                abs(self._adjusted_value(row, "rotation_y_deg")),
                abs(self._adjusted_value(row, "rotation_z_deg")),
            )
            return max(values) >= 2.0
        except (TypeError, ValueError):
            return False

    def _rows_since(self, timestamp):
        if not timestamp or not self._live_path.exists():
            return []
        try:
            ...
            return [
                row for row in rows
                if row.get("timestamp_utc", "") >= timestamp
            ]
        except (OSError, csv.Error):
            return []

    def _update_test_calibration(self):
        rows = self._rows_since(self._calibration_started_at)
        if not rows:
            return
        last_timestamp = rows[-1].get("timestamp_utc")
        if not last_timestamp:
            return
        try:
            start = datetime.fromisoformat(self._calibration_started_at)
            last = datetime.fromisoformat(last_timestamp)
        except (TypeError, ValueError):
            return
        if (last - start).total_seconds() < self._calibration_duration:
            return
        offsets = {}
        for field in self._test_offsets:
            values = []
            for row in rows:
                try:
                    ...
                    continue
            offsets[field] = sum(values) / len(values) if values else 0.0
        self._test_offsets = offsets
        self._calibrating = False
        self._armed = True

    def _adjusted_value(self, row, field):
        return float(row.get(field) or 0) - self._test_offsets.get(field, 0.0)
        ...
        for row in self._recording_rows():
            for name, field in fields.items():
                try:
                    ...
                    continue
                value -= self._test_offsets.get(field, 0.0)
