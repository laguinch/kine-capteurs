...
class KPullAcquisitionService:
    def __init__(self):
        ...
        self._armed = False
        self._csv_path = None
        self._live_path = raw_dir / "kpull_live.csv"
        self._log_path = raw_dir / "kpull_worker.log"
        self._control_path = raw_dir / "kpull_worker_control.json"
    ...
    def disconnect(self):
        with self._lock:
            self._refresh()
            if self._recording:
                raise RuntimeError("Arrêtez le test avant de déconnecter le K-Pull.")
            if self._armed:
                raise RuntimeError("Annulez le test armé avant de déconnecter le K-Pull.")
            if self._process is not None:
    ...
    def start(self, duration=30.0, filename=None):
        with self._lock:
            ...
            if self._process is None:
                raise RuntimeError("Connectez le K-Pull avant de démarrer.")
            ...
            if self._recording or self._armed:
                raise RuntimeError("Une acquisition K-Pull est déjà en cours.")
            if filename is None:
                stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"kpull_{stamp}.csv"
    ...
    def _connection_phase(self):
        ...
        if self._sensor_ready:
            return "ready"
        text = self._read_log()
        if "K-Pull prêt; liaison Bluetooth conservée." in text:
            self._sensor_ready = True
            return "ready"
        if "Tare K-Pull pendant" in text:
            return "tare"
        return "connecting"
...
kpull_service = KPullAcquisitionService()
