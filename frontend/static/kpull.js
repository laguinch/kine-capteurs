...
function update(data) {
  ...
  const labels = {
    disconnected: "K‑Pull déconnecté",
    connecting: "Connexion au K‑Pull",
    tare: "Tare en cours",
    ready: "K‑Pull prêt",
    armed: "En attente de traction",
  ...
  if (data.last_error) {
    ...
    message("Recherche et connexion au K‑Pull…");
  } else if (phase === "tare") {
    message("Laissez le câble du K‑Pull complètement détendu pendant la tare.");
  } else if (phase === "ready") {
    message("Tare terminée. Le K‑Pull est prêt : vous pouvez démarrer le test.");
  } else if (phase === "armed") {
    message("Test prêt. Le chrono démarrera à la première traction.");
  } else if (phase === "active") {
    message("Test en cours : exercez la traction demandée.");
...
async function maybeSave(data) {
  ...
  try {
    const saved = await window.KinePatientSave.saveEvaluation(data, {
      ...
      display_name: label || "Traction maximale",
      summary:
...
async function connect() {
  try {
    await request("/api/kpull/connect", {
      method: "POST",
...
async function disconnect() {
  try {
    await request("/api/kpull/disconnect", { method: "POST" });
  } catch (error) {
    message(error.message, true);
  }
}
...
async function stopTest() {
  ...
  try {
    await request("/api/kpull/stop", { method: "POST" });
  } catch (error) {
    message(error.message, true);
...
async function poll() {
  try {
    const response = await fetch("/api/kpull/latest", { cache: "no-store" });
