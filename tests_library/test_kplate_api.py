...
class KPlateApiTest(unittest.TestCase):
    ...
    def test_routes_are_registered(self):
        ...
        self.assertIn("/kpull/test", paths)
        self.assertIn("/kmove", paths)
        self.assertIn("/kmove/test", paths)
        self.assertIn("/anr-m40", paths)
        self.assertIn("/anr-m40/test", paths)
        ...
        self.assertIn("/api/devices/disconnect", paths)
        ...
        self.assertIn("/api/anr-m40/start", paths)
        self.assertIn("/api/anr-m40/stop", paths)
        self.assertIn("/api/anr-m40/connect", paths)
        self.assertIn("/api/anr-m40/disconnect", paths)
        self.assertIn("/api/anr-m40/latest", paths)
        self.assertIn("/api/anr-m40/download", paths)
    ...
    def test_active_worker_clears_stale_finished_timestamp(self):
        service = DualPlateAcquisitionService()
        service._finished_at = (
        ...
        self.assertTrue(status["running"])
        self.assertIsNone(service._finished_at)
